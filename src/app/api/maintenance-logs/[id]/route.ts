import { auth } from '@clerk/nextjs/server';
import { NextRequest, NextResponse } from 'next/server';
import { createServerClient } from '@/lib/supabase';

// PATCH /api/maintenance-logs/[id] - Update a maintenance log
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const { userId } = await auth();
    if (!userId) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { id } = await params;
    const supabase = createServerClient();
    
    const { data: dbUser } = await supabase
      .from('users')
      .select('id')
      .eq('clerk_id', userId)
      .single();

    if (!dbUser) {
      return NextResponse.json({ error: 'User not found' }, { status: 404 });
    }

    // Verify ownership through component -> boat
    const { data: log } = await supabase
      .from('maintenance_logs')
      .select('*, boat_components!inner(boat_id, boats!inner(owner_id))')
      .eq('id', id)
      .single();

    if (!log || log.boat_components.boats.owner_id !== dbUser.id) {
      return NextResponse.json({ error: 'Maintenance log not found' }, { status: 404 });
    }

    const body = await request.json();

    const { data: updated, error } = await supabase
      .from('maintenance_logs')
      .update({
        maintenance_item: body.maintenance_item || log.maintenance_item,
        date: body.date || log.date,
        description: body.description?.trim() || null,
        cost: body.cost || null,
        currency: body.currency || log.currency,
        hours_at_service: body.hours_at_service || null,
        notes: body.notes?.trim() || null,
      })
      .eq('id', id)
      .select()
      .single();

    if (error) {
      console.error('Error updating maintenance log:', error);
      return NextResponse.json({ error: 'Failed to update' }, { status: 500 });
    }

    return NextResponse.json({ log: updated });
  } catch (error) {
    console.error('PATCH /api/maintenance-logs/[id] error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

// DELETE /api/maintenance-logs/[id]
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const { userId } = await auth();
    if (!userId) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { id } = await params;
    const supabase = createServerClient();
    
    const { data: dbUser } = await supabase
      .from('users')
      .select('id')
      .eq('clerk_id', userId)
      .single();

    if (!dbUser) {
      return NextResponse.json({ error: 'User not found' }, { status: 404 });
    }

    // Verify ownership through component -> boat
    const { data: log } = await supabase
      .from('maintenance_logs')
      .select('*, boat_components!inner(boat_id, boats!inner(owner_id))')
      .eq('id', id)
      .single();

    if (!log || log.boat_components.boats.owner_id !== dbUser.id) {
      return NextResponse.json({ error: 'Maintenance log not found' }, { status: 404 });
    }

    const { error } = await supabase
      .from('maintenance_logs')
      .delete()
      .eq('id', id);

    if (error) {
      console.error('Error deleting maintenance log:', error);
      return NextResponse.json({ error: 'Failed to delete' }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('DELETE /api/maintenance-logs/[id] error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
