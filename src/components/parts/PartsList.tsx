...
export function PartsList({ parts, showComponent = true }: PartsListProps) {
  ...
  return (
    <div className="space-y-3">
      {parts.map((part) => (
        <div key={part.id} className="flex gap-4 p-4 bg-gray-50 rounded-lg">
          {/* Photo */}
          {part.photo_url ? (
            ...
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                {showComponent && part.component_name && (
                  <p className="text-xs text-blue-600">{part.component_name}</p>
                )}
              </div>
              <button
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              ...
              {part.part_number && (
                <div>
                  ...
                  <span className="text-gray-900 font-mono">{part.part_number}</span>
                </div>
              )}
              {part.size_specs && (
                <div>
                  <span className="text-gray-500">Size: </span>
                  <span className="text-gray-900">{part.size_specs}</span>
                </div>
              )}
              {part.supplier && (
                <div>
                  <span className="text-gray-500">Supplier: </span>
                  <span className="text-gray-900">{part.supplier}</span>
                </div>
              )}
            </div>
            
            {part.notes && (
              <p className="mt-2 text-sm text-gray-600 italic">{part.notes}</p>
            )}
          </div>
        </div>
      ))}
