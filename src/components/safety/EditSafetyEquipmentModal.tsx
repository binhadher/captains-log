...
export function EditSafetyEquipmentModal({ isOpen, onClose, equipment, engineType, onSuccess }: EditSafetyEquipmentModalProps) {
  ...
  useEffect(() => {
    if (equipment) {
      ...
      setTypeOther(equipment.type_other || '');
      setQuantity(equipment.quantity || 1);
      setExpiryDate(equipment.expiry_date || '');
      setLastServiceDate(equipment.last_service_date || '');
      setServiceIntervalMonths(equipment.service_interval_months || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/safety-equipment/${equipment.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        throw new Error('Failed to update safety equipment');
      }
      ...
      alert('Failed to update safety equipment');
    } finally {
      setLoading(false);
    }
  };
